...
def cmd_week(args) -> str:
    ...
    if not picks_ids:
        # No completed gameweek to read picks from (pre-season, or a brand new
        # entry). Fall back to a fresh build rather than returning nothing.
        print("No readable picks yet — falling back to build mode.",
              file=sys.stderr)
        squad = build_squad(pool, max_miss=getattr(args, "max_miss", 1))
        xi, bench, _ = best_xi(squad, key="next_gw")
        note = ("_No completed gameweek to read your squad from, so this is a "
                "fresh build. Once GW1 is played, this becomes a real transfer "
                "brief._\n")
        body = [f"# FPL squad build (week fallback) — GW{gw}", "", note,
                render_squad(squad, teams), "", "## Captain shortlist",
                captain_table(xi, teams), ""]
        return "\n".join(body)
